...
- `ios-verify` 工作流含 5 个并行门禁：JS 门禁（单测 + Metro 双端打包）、
  ...
  模拟器冒烟（启动到首页 + 进程判活 + 应用内 13 项自测 + 深链探针）、Android release 回归
...
- 当前进度：设备版未签名 IPA 与模拟器冒烟（启动到首页、进程稳定存活、
  无崩溃、应用内自测 13 项全过）均通过。加密核心（Rust）经桥对齐
  Android 黄金基准、自定义源沙箱、播放、四 Tab 切换、深链（含 `.lxmc`
  导入弹窗）已在模拟器运行时逐项验证，任务进度 23/41，
  详见 `openspec/changes/add-ios-support/tasks.md`。
