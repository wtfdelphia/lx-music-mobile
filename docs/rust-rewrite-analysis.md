...
>
> 分析日期：2026-08-21（2026-08-24 按审核结论修订统计口径）。所有版本号与官方措辞均带出处；查不到的一律标注「未查证」，不以推断充作事实。
>
> 组合路线分析（iOS 适配 × Rust 重构）：[ios-rust-hybrid-analysis.md](./ios-rust-hybrid-analysis.md)。
> **最终决策与执行文档**：[ios-optimal-plan.md](./ios-optimal-plan.md)（综合全部分析的最优 iOS 适配方案）。
...
| UI 代码命运 | 24,787 行 UI 代码 **按平台数翻倍重写** | 24,787 行押注在成熟度不足的框架上 |
...
| 层 | 行数 | 占比 | Rust 可移植性 |
|---|---|---|---|
| `src/screens` + `src/components`（UI，其中 `.tsx` 23,760 行） | 24,787 | 40% | ❌ 不可移植，见 §3 |
| `src/utils/musicSdk`（音源适配） | 10,040 | 16% | ✅ 最适合，见 §2 |
| `src/utils`（除 musicSdk） | 6,607 | 11% | ⚠️ 部分 |
| `src/core`（业务编排） | 5,482 | 9% | ✅ 大部分可移植 |
| `src/theme` | 4,823 | 8% | ⚠️ 其中 4,036 行是生成产物，见下 |
| `src/plugins`（播放/同步/存储） | 2,667 | 4% | ⚠️ 播放部分不可移植 |
| `src/store`（状态） | 2,316 | 4% | ❌ 与 React hooks 绑定 |
| `src/navigation` | 1,787 | 3% | ❌ 绑定 RNN |
| `src/event` | 734 | 1% | ✅ 纯逻辑 |
| **src 合计** | **61,667** | | |
| Android 原生 Java | 3,823 | | 需按平台重建 |

> 口径说明：本表为 `wc -l` 全量口径（含 `.ts`/`.tsx`/`.js`，不含 `.bak`），与 src 合计 61,667 行自洽；未列入目录（`app.ts`、`config/`、`types/` 等）共 2,424 行。
...
| **必须按平台重写 N 遍** | screens + components + navigation + store | ~28,900 |
| **可由生成器重建** | theme 产物 | ~4,000 |
| **原生能力，按平台重建** | Java 5 模块 | 3,823 |
...
| 特性 | 出现次数 | 影响 |
|---|---|---|
| 真实正则调用（`replace(/`、`match(/`、`test(/`、`split(/`） | 82 | 绝大多数是字面匹配与捕获组，`regex` crate 直接支持 |
...
- **弱类型转强类型**：93 处变量下标动态属性访问（`obj[var]`，含常量下标则约 470 处）、46 处 `JSON.parse/stringify`、114 处可选链/空值合并。第三方音乐 API 的响应字段经常缺失或类型不定，JS 里 `a?.b?.c ?? d` 一行搞定，Rust 里要定义 `serde` 结构体并处理 `Option` 嵌套。**这部分代码量通常会膨胀 1.5-2 倍**，且要为每个音源的每个接口定义响应类型。
...
**路线 B（Rust 全栈含 UI）不成立。** 选它等于让本项目去给框架做移动端验证 —— 而这个项目要重写的是 24,787 行已经能正常工作的 UI 代码。
...
推算基于 §1.2 的分组，并采用「重写行数 × 语言膨胀系数」的粗口径。JS→Rust 的膨胀主要来自弱类型转强类型（§2.4 实测：93 处变量下标动态属性访问、46 处 JSON 处理、114 处可选链），经验系数取 1.5-2。
...
| 难点 | 代价 |
|---|---|
| **UI 按平台重写 ×N** | 路线 A 下 24,787 行 UI 代码要按平台数各写一遍；这正是 RN 已经解决的问题（§6.3） |
