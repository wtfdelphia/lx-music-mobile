...
- [ ] 4.5 G1：回归集在 iOS JSC 全量跑，产出通过率报告并按 design.md D6 判读；触发二期则另立 change（前半段已产出：加载→inited 通过率 21/23，逐脚本报告见冒烟日志 `scripts regression` 段，宿主断言脚本同步打印；搜索→取链接阶段与 D6 最终判读待补）
