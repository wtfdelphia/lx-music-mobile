...
| 12 | 本仓库真机反馈（2026-09-02，iPhone 17 Pro / iOS 26.6） | 已收敛到传输段（任务 9.5 → 9.6） | 自定义源本地导入已确认修复生效；切换小枸音乐等源的排行榜加载失败，播放快速循环切歌。两条链路公共依赖 `global.fetch`，该管线在 iOS 上从无运行时证据（CI 出口到音源域名不可达，回归集只测加载→inited）。本地实证排除两条嫌疑：音源端点存活（kw `bang_info` 用本仓库加密契约实测返回 200 并解出完整歌曲列表、tx/mg 榜单端点 200）、上游 `dev` 分支排行榜实现与当前逐字相同（契约未换）。快速循环切歌的机制：取链接失败→5s `addDelayNextTimeout`→`playNext(true)`，秒级失败即秒级切。已补两处失败路径日志（`[request]`/`[userApi request]` 写 error.log）与两项零外网自测（`network_probe`：JSI md5/base64 黄金值 + AbortController/FileReader 全局 + file:// fetch 走生产同管线 + 外网端点软记录；`user_api_request_bridge`：沙箱 `lx.request` 经生产请求链 file:// 往返）。run 33609327722 自测报告给出首份运行时证据：同一管线 `file://` 200（94ms）而外网 `http://qukudata.kuwo.cn` 74ms 即时 `Network request failed`——故障精确落在「RN → NSURLSession 外网传输」段。排除了全部静态嫌疑：无 `NSURLProtocol` 注册、无 `RCTSetCustomNSURLSessionConfigurationProvider`、无 `global.fetch` 覆写、`NO_FLIPPER=1`、ATS 已全放开（`NSAllowsArbitraryLoads` + `NSAllowsLocalNetworking`）。RN Networking 把 NSError 吞成无文本的 `Network request failed`，下一步取证见案例 13 |
| 13 | 本仓库真机反馈（2026-09-02 续报，iPhone 17 Pro / iOS 26.6） | 归因取证中（任务 9.6） | 星海音乐源（`zrcdy.dpdns.org/lx/xinghai-music-sourcev2.3.13.js`，脚本头 `@version v3.2.13`）无法搜索、无法播放。读脚本源码确认：所有出站请求经沙箱 `lx.request` 落回宿主同一条 `global.fetch` 管线，后端域名为 `https://yy.zddyr.top`（fallback `zrcdy.dpdns.org`）——与案例 12 同一条断裂段，症状覆盖面更大（自定义源整条链路而非单个内置源排行榜）。本地实证后端存活：`https://yy.zddyr.top/ip.php` 200、`https://zrcdy.dpdns.org/ip.php` 404（服务端在应答，路径不存在）。修法三件：`UtilsModule.httpProbe` 绕过 RN fetch 栈用原生 `NSURLSession` 直打同一 URL，带回 NSError domain/code/description（RN 把错误吞成无文本的 `Network request failed`，这个探针是唯一能把真实错误带出来的通道，无标记门控，真机正式包可用）；`network_probe` 外网软记录扩为三目标（http qukudata 排行榜族 / https yy.zddyr.top 星海后端 / https apple.com 系统级对照），RN 失败即同步跑原生探针交叉对照；CI 冒烟宿主侧对同三目标跑 `curl` 三角定位（curl 通而应用内失败 → 应用网络栈；两边都不通 → 出口网络，不得归因应用）。判读口径：`external.nativeProbe.ok` 为真即故障在 RN 网络栈配置层，为假则读 NSError 文本进 DNS/ATS/连接归因 |
